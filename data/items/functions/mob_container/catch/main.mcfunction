...
execute as @e[tag=items.mob_container.target] run function lunartech:tools/get_entity_type/main
data modify entity @s Item.tag.entity_type set from storage lunartech:tools entity_type

execute if entity @e[tag=items.mob_container.target,team=NoNames] run data modify entity @s Item.tag.nonames set value 1

execute as @e[tag=items.mob_container.target] run function items:mob_container/catch/get_data/main with storage lunartech:tools
...
execute as @e[tag=items.mob_container.target] run function lunartech:tools/disappear
