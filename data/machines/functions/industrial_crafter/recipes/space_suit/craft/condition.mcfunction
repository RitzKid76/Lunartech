#from ../condition

execute if function machines:industrial_crafter/recipes/space_suit/craft/helmet/condition run return 1
execute if function machines:industrial_crafter/recipes/space_suit/craft/chestplate/condition run return 1
execute if function machines:industrial_crafter/recipes/space_suit/craft/leggings/condition run return 1
execute if function machines:industrial_crafter/recipes/space_suit/craft/boots/condition run return 1

return 0
